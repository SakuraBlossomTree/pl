    ...
    def render(self) -> Panel:
        ...
        if not track:
            content = Align.center(
                Text("No track playing", style="dim italic"), vertical="middle"
            )
            return Panel(
                content,
        ...
        # Status
        loop_indicator = f"[{self.player.loop_mode.upper()}]"
        if self.player.is_playing:
            status = Text(f"> Playing {loop_indicator}", style="status.playing")
        else:
            status = Text(f"|| Paused {loop_indicator}", style="status.paused")
        ...
        content = Group(
            ...
            artist_text,
            Text(),
            Group(Align.center(progress), Align.center(time_text)),
            Text(),
            Group(Align.left(status), Align.right(volume_text)),
        )
    ...
    def move_selection(self, delta: int):
        ...
        if self.player.queue:
            self.selected_index = max(
                0, min(len(self.player.queue) - 1, self.selected_index + delta)
            )
...
class SearchDisplay:
    ...
    def trigger_search(self):
        """Trigger search with current query (for library search tab)."""
    ...
    def render(self, title: str = "Search") -> Panel:
        ...
        query_display.append(
            self.query if self.query else "Type to search...",
        ...
        if self.results:
            ...
            if self.query:
                content = Group(
                    query_display, Text(), Text("No results found", style="dim italic")
                )
            else:
                content = Group(
                    query_display,
    ...
    def move_selection(self, delta: int):
        ...
        if self.results:
            self.selected_index = max(
                0, min(len(self.results) - 1, self.selected_index + delta)
            )
...
class LibraryDisplay:
    ...
    def render(self) -> Panel:
        ...
        if self.view_mode == "tracks":
            info = (
                f" ({len(self.tracks_in_folder)} tracks) - Press BACKSPACE to go back"
            )
        else:
